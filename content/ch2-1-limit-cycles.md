...
$$
\overset{\bullet{}}{x}=x-x^{3}-y-x y^{2}
$$
$$
\overset{\bullet{}}{y}=y-y^{3}+x-y x^{2}
$$
we can plot the vector field in the Cartesian plane along with some trajectories.
![Figure 1](/images/part21/output_001.png)
We see that this system has a center, at $r=1. If \text{ we } \text{ start }$ outside the center, then we spiral towards it. If we start inside the center then we also spiral towards it. The center seems to be attracting trajectories both outside it and inside it.
...
$$
(\text{\textbackslash[NoBreak]}\begin{pmatrix} \overset{\bullet{}}{x} \\ \overset{\bullet{}}{y} \end{pmatrix}\text{\textbackslash[NoBreak]})=A(\text{\textbackslash[NoBreak]}\begin{pmatrix} x \\ y \end{pmatrix}\text{\textbackslash[NoBreak]})
$$
where remember $A$ is 2x2 matrix. Let’s say now that we have some solution given by
$$
(\text{\textbackslash[NoBreak]}\begin{pmatrix} x^{\text{ circ }}(t) \\ y^{\text{ circ }}(t) \end{pmatrix}\text{\textbackslash[NoBreak]})
$$
where this is just some circular trajectory, let’s say with radius $r$. Because of linearity, anything of the form
$$
a (\text{\textbackslash[NoBreak]}\begin{pmatrix} x^{\text{ circ }}(t) \\ y^{\text{ circ }}(t) \end{pmatrix}\text{\textbackslash[NoBreak]}),  a\in{} R
$$
...
$$
\overset{\bullet{} \bullet{}}{x}+\Mu{} \overset{\bullet{}}{x}+x=0
$$
...
$$
A=(\text{\textbackslash[NoBreak]}\begin{pmatrix} 0 & 1 \\ -1 & -\Mu{} \end{pmatrix}\text{\textbackslash[NoBreak]})
$$
...
\InputExpressionUUID->c5ca1dcf-51dc-4e1d-b5ce-ceb166aa4798