...
$$
\dot{x}=-{\partial{}}_{x}V(x,y)
$$
$$
\dot{y}=-{\partial{}}_{y}V(x,y)
$$
...
so that:
$$
\Delta{}V=-{\int{}}_{0}^{T}(\dot{x}. \dot{x})\mathrm{d}t=-{\int{}}_{0}^{T}\dot{|x}|^{2}\mathrm{d}t
$$
...
$$
\dot{x}=-\nabla{}V(x,y), \text{ for } V=-x \text{ sin } y
$$
...
$$
E=\frac{1}{2}(x^{2}+{\dot{x}}^{2})=\frac{1}{2}(x^{2}+y^{2})
$$
where
$$
...
$$
\int{}{\int{}}_{R} ({\partial{}}_{x}v_{x}+{\partial{}}_{y}v_{y})d A={\oint{}}_{\partial{} R}v.n d l     \text{ which is } \text{ equivalent to }:
$$
$$
...
$$
We stated that $\nabla{}. (g(x) \dot{x})$ always took on one sign on $A$ (ie. always positive or always negative), and so the right hand side of this expression is certainly non-zero.
The right hand side can be written as:
$$
{\oint{}}_{C}g(x)( \dot{x}.n )d l
$$
...
$$
\nabla{}. (g(x) \dot{x})={\partial{}}_{x}(e^{-2 x}y)+{\partial{}}_{y}(e^{-2x}(-x-y+x^{2}+y^{2}))=-2 e^{-2x}y+e^{-2 x}(-1+2y)=-e^{-2 x}
$$
