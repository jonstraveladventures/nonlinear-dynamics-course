...
$$
(\text{\textbackslash[NoBreak]}\begin{pmatrix} x_{2} \\ x_{1}-{x_{1}}^{3} \end{pmatrix}\text{\textbackslash[NoBreak]})=(\text{\textbackslash[NoBreak]}\begin{pmatrix} 0.1623 \\ -0.528 \end{pmatrix}\text{\textbackslash[NoBreak]})
$$
...
$$
\Phi{}={\text{ tan }}^{-1}  \frac{{\overset{\bullet{}}{x}}_{2}(x_{1},x_{2})}{  {\overset{\bullet{}}{x}}_{1}(x_{1},x_{2})}
$$
or in the more usual language of $x \text{ and } y:$
$$
\Phi{}={\text{ tan }}^{-1}  \frac{\overset{\bullet{}}{y}(x,y)}{  \overset{\bullet{}}{x} (x,y)}
$$
...
$$
For an example like this, we can say that because $\overset{\bullet{}}{y}$ is always positive, we can never have a vector which is pointing in the negative x direction, and therefore it’s impossible to find any closed path where there is a vector which goes round a full 2Π, so the fixed point at the center here has index 0...different from any of the linear, or indeed linearisable fixed points.
...
$$
\overset{\bullet{}}{z}=z^{n}
$$
For integer $n\in{}\mathbb{Z}$, and $z\in{} \mathbb{C}$ has index $n$. Letting $z=x+i y$, then for, say $n=2, \text{ we } \text{ have }:$
$$
\overset{\bullet{}}{x}+i \overset{\bullet{}}{y}={(x+i y)}^{n}=x^{2}-y^{2}+2i x y
$$
and taking the real and imaginary parts of this gives us the system
$$
\overset{\bullet{}}{x}=x^{2}-y^{2}
$$
$$
\overset{\bullet{}}{y}=2x y
$$
...
For $n=4 \text{ we } \text{ have }$
![Figure 19](/images/part16/output_019.png)
For non-integer values of $n \text{ we } \text{ end } \text{ up } \text{ with } $so-called branch-cuts and so the fixed point is no longer isolated so it doesn’t make sense to talk about the index of the fixed point.
...
We have:
$$
\Phi{}=\text{ arctan }(\frac{\overset{\bullet{}}{y}}{\overset{\bullet{}}{x}})=\text{ arctan }(\frac{Im(\overset{\bullet{}}{z})}{Re(\overset{\bullet{}}{z})})=\text{ arctan }(\frac{Im(z^{n})}{Re(z^{n})})=\text{ arctan }(\frac{Im(r^{n} e^{i n \Theta{}})}{Re(r^{n} e^{i n \Theta{}})})=\text{ arctan }(\frac{\text{ sin }(n \Theta{})}{\text{ cos }(n \Theta{})})=n \Theta{}
$$
So
$$
