...
0=x_{1}-{x_{1}}^{3}
$$
which is solved by $(x_{1},x_{2})=(-1,0), (1,0) and (0,0). $And indeed this does correspond to the fixed points.
...
{\dot{x}}_{2}=x_{1}-{x_{1}}^{3}
$$
We could ask what is happening at the point $(2,2) and we$’d find that:
$$
{\dot{x}}_{1}=2
$$
$$
{\dot{x}}_{2}=2-8=-6
$$
So we would put an arrow pointing in the $(2,-6) direction.$ Doing this over a region of the phase plane we would get:
...
$$
\dot{x}\text{\textbackslash[TildeTilde]}\frac{x_{n+1}-x_{n}}{\text{\textbackslash[CapitalDelta]}t}
$$
...
$$
\frac{x_{n+1}-x_{n}}{\text{\textbackslash[CapitalDelta]}t}=f(x_{n})
$$
the choice here to make the function be evaluated at $x_{n}$ is actually where we lose a bit of accuracy. In some senses, we could have chosen to evaluate it at $x_{n} or x_{n+1}$ or perhaps an average of the two: $\frac{x_{n}+x_{n+1}}{2}$. We will see below how to improve this.
Taking the naive approach we can write:
$$
x_{n+1}=f(x_{n})\text{\textbackslash[CapitalDelta]}t+x_{n}
$$
...
$$
x_{n+1}=x_{n}+\frac{(k_{1}+2k_{2}+2k_{3}+k_{4})}{6}
$$
$$
k_{1}=f(x_{n})\text{\textbackslash[CapitalDelta]}t
$$
$$
k_{2}=f(x_{n}+\frac{1}{2}k_{1})\text{\textbackslash[CapitalDelta]}t
$$
$$
k_{3}=f(x_{n}+\frac{1}{2}k_{2})\text{\textbackslash[CapitalDelta]}t
$$
$$
k_{4}=f(x_{n}+k_{3})\text{\textbackslash[CapitalDelta]}t
$$
...
Give this a go for the example plot above, using Δt=0.01 and try and start at $x_{0}=(0,0.5)$ and see what you get.
Exercise
Take the equations
$$
\dot{x}=x+e^{-y}
...
\dot{y}=-y
$$
(where here we have used $x and y instead of x_{1} and x_{2}$).
